...
const DesktopControls: FC<{
  ...
  const ref = useRef<THREE.Camera | null>(null);
  useFrame(() => {
    if (!ref.current) return;
    (ref.current as any).target?.copy(pivot);
  });
  return (
    <OrbitControls
      ref={ref as any}
...
const ModelInner: FC<ModelInnerProps> = ({
  ...
  useLayoutEffect(() => {
    ...
    const recenterBox = new THREE.Box3().setFromObject(g);
    const recenter = recenterBox.getCenter(new THREE.Vector3());
    g.position.sub(recenter);
  ...
  useFrame((_, dt) => {
    ...
    const ppx = cPar.current.x;
    const ppy = cPar.current.y;
    ...
    if (enableMouseParallax) {
      outer.current.rotation.x += cPar.current.y - ppy;
      outer.current.rotation.y += cPar.current.x - ppx;
    }
